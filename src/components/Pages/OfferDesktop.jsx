...
export const OfferDesktop = (props) => {
  ...
  return (
    <div className={` p-5 relative mx-3 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      <div className={`flex items-center mb-4 ${darkMode ? 'text-white ' : ''}`}>
        <img src={offer.logo} alt='Company Logo' className='w-10 h-10 rounded-full mr-3' />
        <div>
          <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{offer.position}</h2>
        ...
        <span className='font-bold'>
          ...
          <ul className='mt-2'>
            {offer.technologies.map((technology, index) => {
              return (
                <li className={`flex items-center text-sm ${darkMode ? 'text-white' : ''}`} key={index}>
      ...
      <button className={`bg-blue-500 flex m-auto py-2 px-10 mt-8 mb-6 text-white`}>
        Apply
      </button>
    </div>
  );
