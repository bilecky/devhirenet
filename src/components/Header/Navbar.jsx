...
const Navbar = () => {
  ...
  return (
    <div className={`bg-gray-800 max-h-screen ${shouldRenderHandler ? 'h-auto' : 'lg:h-30vh'}`}>
      <Wrapper>
        <header className="flex items-center justify-between text-gray-900">
          <Link to="/">
            <div className="lg:text-4xl flex items-center text-gray-50">
              <ImBlog />
              <span className="ml-3 text-2xl lg:text-4xl font-oswald">DevHireNet</span>
            </div>
          </Link>

          <div className="flex items-center text-gray-50">
            <button className="mr-6 text-yellow-500 focus:outline-none" onClick={toggleDarkMode}>
              <FiMoon size={24} />
            </button>
            <Link to="/favorites" className="flex items-center mr-6" onClick={handleFavoritesClick}>
              <FiStar size={24} />
              {isAuthenticated && <span className="ml-2 text-lg">{likedOffers?.length}</span>}
            </Link>
